...
typedef struct master_t {
    ...
    uint8_t flaps_type;                     // airplane mode flaps, 0 = no flaps, 1 = flaps enabled, 2 = flaperons enabled, 3 = flaperons enabled reversed direction
    uint8_t flaperon_channel;               // Flaperon channel
    uint8_t flaps_speed;                    // airplane mode flaps/flaperons speed, 0 = unlimited flaps speed, > 0 = flap speed, larger = faster
    uint16_t minflaperons;                  // Set the minimum flaperons endpoint 1000..2000
    uint16_t maxflaperons;                  // Set the maximum flaperons endpoint 1000..2000
