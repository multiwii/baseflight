...
static bool gpsNewFrameNMEA(char c)
{
    ...
    static int32_t GPS_temp[2];
    static uint8_t GPS_numSat_temp;
    static uint16_t GPS_altitude_temp, GPS_speed_temp;
    static uint16_t GPS_ground_course_temp;
...
//              break;
            case 2:
                GPS_temp[LAT] = GPS_coord_to_degrees(string);
                break;
            case 3:
                if (string[0] == 'S')
                    GPS_temp[LAT] *= -1;
                break;
            case 4:
                GPS_temp[LON] = GPS_coord_to_degrees(string);
                break;
            case 5:
                if (string[0] == 'W')
                    GPS_temp[LON] *= -1;
            ...
            case 6:
                ...
                break;
            case 7:
                GPS_numSat_temp = grab_fields(string, 0);
                break;
            case 9:
                GPS_altitude_temp = grab_fields(string, 0);     // altitude in meters added by Mis
                break;
            }
            break;
        case FRAME_RMC:        //************* GPRMC FRAME parsing
            switch (param) {
            case 7:
                GPS_speed_temp = ((grab_fields(string, 1) * 5144L) / 1000L);    // speed in cm/s added by Mis
                break;
            case 8:
                GPS_ground_course_temp = (grab_fields(string, 1));      // ground course deg * 10
                break;
            }
            break;
        }
        ...
        if (checksum_param) {   //parity checksum
            ...
            if (checksum == parity) {
                ...
                case FRAME_GGA:
                  frameOK = 1;
                  if (f.GPS_FIX) {
                        GPS_coord[LAT] = GPS_temp[LAT];
                        GPS_coord[LON] = GPS_temp[LON];
                        GPS_numSat = GPS_numSat_temp;
                        GPS_altitude = GPS_altitude_temp;
                    }
                    break;
                case FRAME_RMC:
                    GPS_speed = GPS_speed_temp;
                    GPS_ground_course = GPS_ground_course_temp;
                    break;
                } // end switch
            }
        }
