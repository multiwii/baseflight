...
void gpsInit(uint8_t baudrateIndex)
{
    ...
    // we dont start gps
    // if the port is in use for msp
    if (mcfg.gps_port == mcfg.msp_port)
        return;

    // or if the port is in use for serial rx
    if (feature(FEATURE_SERIALRX) && mcfg.gps_port == SERIALPORT_UART_2)
        return;
