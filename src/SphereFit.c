        ...
        float SphereOrigin[], float * SphereRadius)
{
    ...
    do {
        ...
        Delta[X] = -((F2 + 16.0f * (C[Y] * XY + C[Z] * XZ + sum[X] * (-C2[X] - Q0) + C[X] * (sum2[X] + Q1 - C[Z] * sum[Z] - C[Y] * sum[Y]))) / Denom[X]);
        Delta[Y] = -((F3 + 16.0f * (C[X] * XY + C[Z] * YZ + sum[Y] * (-C2[Y] - Q0) + C[Y] * (sum2[Y] + Q1 - C[X] * sum[X] - C[Z] * sum[Z]))) / Denom[Y]);
        Delta[Z] = -((F4 + 16.0f * (C[X] * XZ + C[Y] * YZ + sum[Z] * (-C2[Z] - Q0) + C[Z] * (sum2[Z] + Q1 - C[X] * sum[X] - C[Y] * sum[Y]))) / Denom[Z]);
        ...
        Iterations++;
    } while ((Iterations < 50) || ((Iterations < MaxIterations) && ((Sqr(Delta[X]) + Sqr(Delta[Y]) + Sqr(Delta[Z])) > Err)));
